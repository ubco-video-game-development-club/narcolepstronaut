...
public class ShadowManager : MonoBehaviour {
	...
	void Start ()
	{
		...
		{
			if (planet[i].GetComponent<ShadedPlanet>())
			{
				int		x = -1;
				int		x2 = -1;
				planet[i].GetComponent<ShadedPlanet>().setShadowNumber(planet.Length - 1); // first we need to set the number of object we want ( - 1 because we wont give own coordinates)
				while (++x < planet.Length) // giving each planet the coordinates of all the other planet
				{
					...
					{
						x2++;
						planet[i].GetComponent<ShadedPlanet>().setShadow(planet[x].transform, x2);
					}
				}
			}
		}
